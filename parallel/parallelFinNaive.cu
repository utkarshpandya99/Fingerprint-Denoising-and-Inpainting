...
__global__ void ndConvolutionGradient(double *paddedGxx, double *paddedGyy, double *paddedGxy, double *Gxx, double *Gyy, double *Gxy, double *GKernelBlock, int gradientPaddedWidth, int kernelSize, int width, int height){
    int i, j;
...
__global__ void ndConvolutionSinCos(double *paddedsin2theta, double *paddedcos2theta, double *GKernelOrient, double *sin2theta, double *cos2theta, int gradientPaddedWidth, int kernelSize, int width, int height){
    int i, j;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(){
    ...
    double *dev_paddedGxx,*dev_paddedGyy,*dev_paddedGxy,*dev_Gxx,*dev_Gyy,*dev_Gxy,*dev_GKernelBlock;
    ...
    cudaMalloc((void**)&dev_GKernelBlock,kernelSize*kernelSize*sizeof(double));
    ...
    cudaMemcpy(dev_GKernelBlock,GKernelBlock,kernelSize*kernelSize*sizeof(double),cudaMemcpyHostToDevice);

    ndConvolutionGradient<<<blocksGradient, threadsGradient>>>(dev_paddedGxx,dev_paddedGyy,dev_paddedGxy,dev_Gxx,dev_Gyy,dev_Gxy,dev_GKernelBlock,gradientPaddedWidth,kernelSize,width,height);
    ...
    double *dev_paddedsin2theta,*dev_paddedcos2theta,*dev_sin2theta,*dev_cos2theta,*dev_GKernelOrient;
    ...
    cudaMalloc((void**)&dev_GKernelOrient,kernelSize*kernelSize*sizeof(double));
    ...
    cudaMemcpy(dev_GKernelOrient,GKernelOrient,kernelSize*kernelSize*sizeof(double),cudaMemcpyHostToDevice);

    ndConvolutionSinCos<<<blocksOrient, threadsOrient>>>(dev_paddedsin2theta,dev_paddedcos2theta,dev_GKernelOrient,dev_sin2theta,dev_cos2theta,gradientPaddedWidth,kernelSize,width,height);
    cudaDeviceSynchronize();
