...
__global__ void ndConvolutionGradient(double *paddedGxx, double *paddedGyy, double *paddedGxy, double *Gxx, double *Gyy, double *Gxy, int gradientPaddedWidth, int kernelSize, int width, int height){
    ...
    extern __shared__ double s_arrayxx[];
    extern __shared__ double s_arrayyy[];
    extern __shared__ double s_arrayxy[];
    int smsize = 32 + kernelSize -1;

    int npaddedrow = blockDim.y + kernelSize -1;
    int npaddedcol = blockDim.x + kernelSize -1;
    
    int offsetrow = threadIdx.y + blockDim.y;
    int offsetcol = threadIdx.x + blockDim.x;

    int goffsetrow = blockDim.y * blockIdx.y + offsetrow;
    int goffsetcol = blockDim.x * blockIdx.x + offsetcol;

    s_arrayxx[threadIdx.y*smsize +threadIdx.x] = paddedGxx[row*gradientPaddedWidth+col];
    s_arrayyy[threadIdx.y*smsize +threadIdx.x] = paddedGyy[row*gradientPaddedWidth+col];
    s_arrayxy[threadIdx.y*smsize +threadIdx.x] = paddedGxy[row*gradientPaddedWidth+col];

    if (offsetrow<npaddedrow && offsetcol<npaddedcol){
        s_arrayxx[offsetrow*smsize+offsetcol]=paddedGxx[goffsetrow*gradientPaddedWidth+goffsetcol];
        s_arrayyy[offsetrow*smsize+offsetcol]=paddedGyy[goffsetrow*gradientPaddedWidth+goffsetcol];
        s_arrayxy[offsetrow*smsize+offsetcol]=paddedGxy[goffsetrow*gradientPaddedWidth+goffsetcol];
    }
    __syncthreads();
    ...
    if (row<height && col<width){
        ...
        for (i=-(kernelSize/2);i<=(kernelSize/2);i++){
            for(j=-(kernelSize/2);j<=(kernelSize/2);j++){
                sumxx+=s_arrayxx[(threadIdx.y+i+(kernelSize/2))*smsize+(threadIdx.x+j+(kernelSize/2))]*mask_gradient[(i+(kernelSize/2))*kernelSize+(j+(kernelSize/2))];
                sumyy+=s_arrayyy[(threadIdx.y+i+(kernelSize/2))*smsize+(threadIdx.x+j+(kernelSize/2))]*mask_gradient[(i+(kernelSize/2))*kernelSize+(j+(kernelSize/2))];
                sumxy+=s_arrayxy[(threadIdx.y+i+(kernelSize/2))*smsize+(threadIdx.x+j+(kernelSize/2))]*mask_gradient[(i+(kernelSize/2))*kernelSize+(j+(kernelSize/2))];
...
__global__ void ndConvolutionSinCos(double *paddedsin2theta, double *paddedcos2theta, double *sin2theta, double *cos2theta, int gradientPaddedWidth, int kernelSize, int width, int height){
    ...
    extern __shared__ double s_arraysin2theta[];
    extern __shared__ double s_arraycos2theta[];
    int smsize = 32 + kernelSize -1;
    ...
    s_arraysin2theta[threadIdx.y*smsize +threadIdx.x] = paddedsin2theta[row*gradientPaddedWidth+col];
    s_arraycos2theta[threadIdx.y*smsize +threadIdx.x] = paddedcos2theta[row*gradientPaddedWidth+col];

    if (offsetrow<npaddedrow && offsetcol<npaddedcol){
        s_arraysin2theta[offsetrow*smsize+offsetcol]=paddedsin2theta[goffsetrow*gradientPaddedWidth+goffsetcol];
        s_arraycos2theta[offsetrow*smsize+offsetcol]=paddedcos2theta[goffsetrow*gradientPaddedWidth+goffsetcol];
    }
    __syncthreads();
    ...
    if (row<height && col<width){
        ...
        for (i=-(kernelSize/2);i<=(kernelSize/2);i++){
            for(j=-(kernelSize/2);j<=(kernelSize/2);j++){
                sumsin+=s_arraysin2theta[(threadIdx.y+i+(kernelSize/2))*smsize+(threadIdx.x+j+(kernelSize/2))]*mask_orient[(i+(kernelSize/2))*kernelSize+(j+(kernelSize/2))];
                sumcos+=s_arraycos2theta[(threadIdx.y+i+(kernelSize/2))*smsize+(threadIdx.x+j+(kernelSize/2))]*mask_orient[(i+(kernelSize/2))*kernelSize+(j+(kernelSize/2))];
            }
        }
...
__global__ void gaborConvolution(double *freqIm, double *orientIm, double *normIm, double *gaborFilter, int *gaborIndexValueList, int gaborRows, int gaborCols, int gaborHalfSize,int gradientPaddedWidth, int gradientPaddedHeight, int width, int height){
    int i,j;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;

    // extern __shared__ double s_arraynormim[];
    // int smsize = 32 + 2*gaborHalfSize;

    // int npaddedrow = blockDim.y + 2*gaborHalfSize;
    // int npaddedcol = blockDim.x + 2*gaborHalfSize;
    ...
    // s_arraynormim[threadIdx.y*smsize +threadIdx.x] = normIm[row*gradientPaddedWidth+col];

    // if (offsetrow<npaddedrow && offsetcol<npaddedcol){
    //     s_arraynormim[offsetrow*smsize+offsetcol]=normIm[goffsetrow*gradientPaddedWidth+goffsetcol];
    // }
    // __syncthreads();
    ...
    //                     gabor_sum+=s_arraynormim[(threadIdx.y+i+gaborHalfSize)*smsize+(threadIdx.x+j+gaborHalfSize)]*gaborFilter[ ((ang_i)*gaborRows*gaborCols) + ((gaborHalfSize+i)*gaborCols) + (gaborHalfSize+j)];
    //                 }
    //             }
    //             if(gabor_sum<-3){
    //                 gaborIndexValueList[(row*width+col)*3+0]=row;
...
int main(){
    ...
    size_t SHMEM_Gradient = (T+kernelSize-1)*(T+kernelSize-1)*sizeof(double);
    ...
    ndConvolutionGradient<<<blocksGradient, threadsGradient, SHMEM_Gradient>>>(dev_paddedGxx,dev_paddedGyy,dev_paddedGxy,dev_Gxx,dev_Gyy,dev_Gxy,gradientPaddedWidth,kernelSize,width,height);
    ...
    size_t SHMEM_Orient = (T+kernelSize-1)*(T+kernelSize-1)*sizeof(double);
    ...
    ndConvolutionSinCos<<<blocksOrient, threadsOrient, SHMEM_Orient>>>(dev_paddedsin2theta,dev_paddedcos2theta,dev_sin2theta,dev_cos2theta,gradientPaddedWidth,kernelSize,width,height);
    cudaDeviceSynchronize();
    ...
    // gradientPaddedWidth = width + gaborCols -1;
    // gradientPaddedHeight = height + gaborRows -1;

    // double *paddedNormImGabor = (double *)malloc(gradientPaddedWidth*gradientPaddedHeight*sizeof(double));
    ...
    // paddingZeroesOnEdges(paddedNormImGabor,normIm,gaborRows,gradientPaddedWidth,gradientPaddedHeight,width,height);

    dim3 blocksGabor(Bwidth,Bheight);
    dim3 threadsGabor(T,T);

    // size_t SHMEM_Gabor = (T+ 2*gaborHalfSize)*(T+ 2*gaborHalfSize)*sizeof(double);
    ...
    gaborConvolution<<<blocksGabor,threadsGabor>>>(dev_freqIm,dev_orientIm,dev_normIm,dev_gaborFilter,dev_gaborIndexValueList,gaborRows,gaborCols,gaborHalfSize,gradientPaddedWidth,gradientPaddedHeight,width,height);
    cudaDeviceSynchronize();
