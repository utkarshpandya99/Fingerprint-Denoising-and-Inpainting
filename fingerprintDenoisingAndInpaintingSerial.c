...
void ndconvolutionsincos(double *paddedGxx, double *paddedGyy, double *paddedGxy, double *sin2theta, double *cos2theta, double *GaussKernelBlockSigma, int gausskersize, int paddedwidth, int paddedheight, int width, int height){
    int row, col, i, j;
    for(row=0;row<height;row++){
        for(col=0;col<width;col++){
            double sumxx=0.0;
            ...
            sumxy = 2*sumxy;
            denom = sqrt(pow(sumxy,2)+pow((sumxx-sumyy),2)) + DBL_EPSILON;
...
void ndconvolutionorient(double *paddedsin2theta, double *paddedcos2theta, double *orientim, double *GaussKernelBlockSigma, int gausskersize, int paddedwidth, int paddedheight, int width, int height){
    int row, col, i, j;
    for(row=0;row<height;row++){
        for(col=0;col<width;col++){
            double sumsin=0.0;
            ...
            for (i=-(gausskersize/2);i<=(gausskersize/2);i++){
                for(j=-(gausskersize/2);j<=(gausskersize/2);j++){
                    sumsin+=paddedsin2theta[(row+i+(gausskersize/2))*paddedwidth+(col+j+(gausskersize/2))]*GaussKernelBlockSigma[(i+(gausskersize/2))*gausskersize+(j+(gausskersize/2))];
                    sumcos+=paddedcos2theta[(row+i+(gausskersize/2))*paddedwidth+(col+j+(gausskersize/2))]*GaussKernelBlockSigma[(i+(gausskersize/2))*gausskersize+(j+(gausskersize/2))];
                }
            }
            orientim[row*width+col] = (PI/2.0) + (atan2(sumsin,sumcos)/2.0);
        }
    }
}
...
void ridgefrequency(double *normim, double *orientim, bool *mask, double *freqim, int blocksizefreq, double *blockimagefreq, double *orientblock, double *proj, double *proj2, bool *maxpts, int windowsizefreq, int minwavelength, int maxwavelength, int width, int height){
    ...
    //moves into blocks and leaps one block after every iteration
    for(row=0;row<height-blocksizefreq;row+=blocksizefreq){
        for(col=0;col<width-blocksizefreq;col+=blocksizefreq){
            ...
            //extracts information blockwise
            ...
            //sums the pixel values down the columns
            for (i=0;i<blocksizefreq;i++){
                for (j=0;j<blocksizefreq;j++){
            ...
            //greyscale dilation
            //runs a window in the 1d matrix of sum of columns with current pixel at center and 
            //assigns value that is highest in that window +1
            //for example, 1 2 1 4 5 6 7 8 is the matrix and window size if 5
            //  1 1 1 1 1            -->window
            //      1 2 1 | 4 5 6 7 8  --> matrix (first element being the center one right now)
            //      2 3 2 | 4 5 6 7 8  --> sum of window + matrix
            //  2 3 and 2 are values in window, highest is 3 and hence assigns 3 to index one and so on
            ...
            //greyscale dilated matrix proj2(dilated proj) - proj(sum down the columns) and its absolute value
            for(i=0;i<blocksizefreq;i++){
            ...
            //finds peak for one wave of ridge and valley of the fingerprint
            //its done by finding whenever the proj value is above average but its dilated value change less than threshold
            //so that the pixel does not have too much change from the original value, which means it is a ridge and not an aberration
            colsmaxind=0;
            for(i=0;i<blocksizefreq;i++){
            ...
            //makes a note of all the ridges
            ...
            //if there are more than 2 ridges and if wavelengths are acceptable it calcuulates freq and assigns it to block
            //to only segmented image
            if(colsmaxind>=2){
...
void create3dgabor(double *gaborfilter3d, double *gaborfilter, double *rotatedgabor, double *rotind, double angleinc, int gabfiltersize){
    ...
    for(angle=0;angle<(round(180/angleinc));angle++){
        ...
        for(i=0;i<(gabfiltersize*gabfiltersize);i++){
            rotind[i*3+0]=rotind[i*3+0]-(rotind[(int)round((gabfiltersize*gabfiltersize)/2)*3+0]-(gabfiltersize/2));
            rotind[i*3+1]=rotind[i*3+1]-(rotind[(int)round((gabfiltersize*gabfiltersize)/2)*3+1]-(gabfiltersize/2));
        }
        ...
        //stores rotated gabor filter in the final matrix from the intermediate one
        for(i=0;i<gabfiltersize;i++){
            for(j=0;j<gabfiltersize;j++){
...
void gaborconvolution(double *gaborfilter3d, double *finalim, double *freqim,double *normim, double *orientim, bool *mask, int *gaborind, int gabfiltersize, int gaborsize, int width, int height){
    int row, col, ang, i, j;
    double gaborsum;
    printf("\ngaborsize = %d",gaborsize);
    for(row=gaborsize;row<(height-gaborsize);row++){
        for(col=gaborsize;col<(width-gaborsize);col++){
...
int main(){
    ...
    int intmax, intmin, intsum; //integer max, min and sum variables
    int length,gaborsize,gabfiltersize,maxorientindex;
    int histpercent, totalrelevantpixels, gradientsigma, blocksigma, orientsmoothsigma, gausskersize;
    ...
    gausskersize=round(6*gradientsigma)+1;
    ...
    //block sigma 
    blocksigma=7;
    gausskersize=round(6*blocksigma)+1;
    ...
    ndconvolutionsincos(paddedGradientxx,paddedGradientyy,paddedGradientxy,sin2theta,cos2theta,GaussianKernelBlockSigma,gausskersize,paddedgausswidth,paddedgaussheight,width,height);
    ...
    ndconvolutionorient(paddedsin2theta,paddedcos2theta,orientim,GaussianKernelBlockSigma,gausskersize,paddedgausswidth,paddedgaussheight,width,height);
    ...
    //Ridge Frequency
    //==============================================================================================
    blocksizefreq=38;       //bigger block size to scan for freq blockwise
    windowsizefreq=5;       //to implement greyscale dilation in the sum of column pixel value matrix
    minwavelength=5;        //min acceptable wavelength
    maxwavelength=15;       //max acceptable wavelength
    ...
    //finds mean freq for relevant pixels
    ...
    meanfreq=(double)(round(meanfreq*100)/100);  //rounds mean freq to 0.01 decimal value
    sigmagabor=1.0/meanfreq*kgabor;              //sigma for gabor filter
    gaborsize=round(3*sigmagabor);               //size of one span of gabor filter
    gabfiltersize=2*gaborsize+1;                 //size/width/height of gabor filter
    ...
    //3d matrix for rotated gabor filters at every 3 degree rotation
    double *gaborfilter3d = (double *)malloc(((int)(180/angleinc))*gabfiltersize*gabfiltersize*sizeof(double));
    double *rotatedgabor=(double*)malloc(gabfiltersize*gabfiltersize*sizeof(double)); //intermediate rotated gabor filter
    double *rotind=(double*)malloc(gabfiltersize*gabfiltersize*3*sizeof(double)); //indices matrix with corresponding values
    ...
    //rounds orient matrix with indices values of 3d gabor filter matrix corresponding to nth angle rotated filter
    roundorientation(orientim,angleinc,maxorientindex,width,height);

    int *gaborind = (int*)malloc(width*height*3*sizeof(int)); //stores intermediate values of gabor filter for rotation
    ...
    gaborconvolution(gaborfilter3d,finalim,freqim,normim,orientim,mask,gaborind,gabfiltersize,gaborsize,width,height);

    //==============================================================================================

    end=clock();
    walltime = (end-start)/(double)CLOCKS_PER_SEC;
    printf("Walltime = %lf", walltime);


    //assigning values from index matrix to output final image
    assigningoutput(finalim,gaborind,width,height);

    //stores output in the image matrix to be writ in pgm file
